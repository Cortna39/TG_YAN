...
def send(payload: dict):
    try:
        ...
        r.raise_for_status()
        log.info("mp_sent", extra={"counter": payload.get("tid"), "event": payload.get("ea"), "deal": payload.get("ti")})
        return True
    except Exception as e:
        log.error("mp_failed", extra={"counter": payload.get("tid"), "event": payload.get("ea"), "deal": payload.get("ti"), "error": str(e)})
        raise
